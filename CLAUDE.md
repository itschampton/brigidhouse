...
- Media kit stats on `work-with-me/index.html` are pulled manually from TikTok analytics screenshots the owner provides, dated in the page header (`Updated <date>`) — there's no live data feed, so refreshing the numbers means editing the HTML directly and updating that date. Convention as of the last update: a 60-day window for post views/viewers/likes/comments/shares (matching the owner's TikTok analytics tab), with no delta/change indicators shown — just the plain number and label, no `stat-delta` (that CSS class was removed as unused when deltas were dropped at the owner's request). Followers is the one exception: it's a cumulative all-time count, not tied to the 60-day window. Home page's About Me mini-stats mirror the same views/likes numbers and window as the media kit page — keep them in sync when updating either.
...
- `404.html` (root): GitHub Pages automatically serves this for any unmatched path — required naming/location, don't move it. Styled to match the rest of the site (same nav, fonts, `.hero`/`.socials`/footer patterns), single "Back to home" button, `<meta name="robots" content="noindex">` so it can't get indexed as a real page.
- `robots.txt` and `sitemap.xml` (root): basic SEO hygiene, listing both real pages (`/` and `/work-with-me/`). `404.html` is intentionally left out of the sitemap. If a new page is ever added, add it to `sitemap.xml` too.
